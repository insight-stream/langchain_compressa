    ...
    if isinstance(message, ChatMessage):
        ...
        if message.tool_calls or message.invalid_tool_calls:
            message_dict["tool_calls"] = [
                _lc_tool_call_to_compressa_tool_call(tc) for tc in message.tool_calls
            ] + [
                _lc_invalid_tool_call_to_compressa_tool_call(tc)
...
class BaseChatCompressa(BaseChatModel):
    ...
    """Base URL path for API requests, leave blank if not using a proxy or service 
        emulator."""
    compressa_proxy: Optional[str] = None
    ...
    be the same as the embedding model name."""
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        values["compressa_api_key"] = convert_to_secret_str(
            get_from_dict_or_env(values, "compressa_api_key", "COMPRESSA_API_KEY")
        )
        ...
        values["compressa_proxy"] = get_from_dict_or_env(
            values,
            "compressa_proxy",
            "COMPRESSA_PROXY",
            default="",
        ...
        compressa_proxy = values["compressa_proxy"]
        if not values.get("client"):
            if compressa_proxy and not values["http_client"]:
                try:
                    import httpx
                ...
                values["http_client"] = httpx.Client(proxy=compressa_proxy)
            sync_specific = {"http_client": values["http_client"]}
            values["client"] = openai.OpenAI(
                **client_params, **sync_specific
            ).chat.completions
        if not values.get("async_client"):
            if compressa_proxy and not values["http_async_client"]:
                try:
                    import httpx
                except ImportError as e:
                    raise ImportError(
                ...
                values["http_async_client"] = httpx.AsyncClient(proxy=compressa_proxy)
    ...
    def bind_functions(
        ...
        **kwargs: Any,
        ...
        Assumes model is compatible with Compressa function-calling API.
    ...
    def bind_tools(
        ...
        **kwargs: Any,
        ...
        Assumes model is compatible with Compressa tool-calling API.
        ...
        **kwargs: Any,
        ...
        Args:
            schema: The output schema as a dict or a Pydantic class. If a Pydantic class
                ...
                must match the Compressa function-calling spec or be a valid JSON schema
                with top level 'title' and 'description' keys specified.
            method: The method for steering model generation, either "function_calling"
                or "json_mode". If "function_calling" then the schema will be converted
                to an Compressa function and the returned model will make use of the
                function-calling API. If "json_mode" then Compressa's JSON mode will be
        ...
        Example: Function-calling, Pydantic schema (method="function_calling", include_raw=False):
            .. code-block:: python
                ...
                llm = ChatCompressa(model="/app/resources/models/models/compressa-ai_Llama-3-8B-Instruct", temperature=0)
...
class ChatCompressa(BaseChatCompressa):
    """Compressa chat model integration.
    ...
    def get_lc_namespace(cls) -> List[str]:
        ...
        return ["langchain", "chat_models", "compressa"]
...
def _lc_tool_call_to_compressa_tool_call(tool_call: ToolCall) -> dict:
    return {
        "type": "function",
        "id": tool_call["id"],
        "function": {
...
def _lc_invalid_tool_call_to_compressa_tool_call(
